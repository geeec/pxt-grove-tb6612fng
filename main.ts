 ...
 *
 * Note : la commande CMD_STANDBY (0x04) active le driver,
 * et CMD_NOT_STANDBY (0x05) le désactive.
 * C'est le comportement constaté et identique à l'exemple Arduino
 * où init() appelle standby() avant d'utiliser les moteurs.
 */
...
namespace groveMoteur {
     ...
     * Trame I2C : [START, addr+W, cmd, data, STOP]
     */
    function envoyerCommande(cmd: number, data: number): void {
        ...
        pins.i2cWriteBuffer(ADRESSE_I2C, buf, false)
        basic.pause(10)
    }
     ...
     * Envoie la commande STANDBY (0x04) qui active le driver
     * (identique à la séquence Arduino : init() appelle standby()).
     */
